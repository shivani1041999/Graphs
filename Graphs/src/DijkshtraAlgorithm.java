...
public class DijkshtraAlgorithm {
	private static void dijkshtra(int[][] input) {
		int V = input.length;
		//visited array
		...
		for(int i = 0; i < V; i++) {
			...
			for(int j = 0; j < V; j++) {
				if(!vis[j] && input[minvertex][j] != 0) {
					if(weight[j] > input[minvertex][j] + weight[minvertex]) {
						weight[j] = input[minvertex][j] + weight[minvertex];
					}
				}
			}
		}
		for(int v = 1; v < V; v++) {
				System.out.println(v + " " + weight[v]);
		}
